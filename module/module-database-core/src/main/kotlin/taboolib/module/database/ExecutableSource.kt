 ...
 */
open class ExecutableSource(val table: Table<*, *>, var dataSource: DataSource) {
            ...
            override fun <C> call(func: ResultSet.() -> C): C {
                return dataSource.connection.use {
                    try {
                        ...
                        prepareStatement.executeQuery().use { func(this) }.also { action?.callFinally(prepareStatement, this@use) }
            ...
            override fun <C> call(func: ResultSet.() -> C): C {
                return dataSource.connection.use {
                    try {
                        val prepareStatement = prepareStatement(query, autoGeneratedKeys)
                        action?.elements?.forEachIndexed { index, any -> prepareStatement.setObject(index + 1, any) }
                        (prepareStatement.executeUpdate() as C).also { action?.callFinally(prepareStatement, this@use) }
                    } catch (ex: SQLException) {
                        warning(query)
                        warning("Statement parameter (${action?.elements?.size ?: 0}): ${action?.elements}")
                        throw ex
