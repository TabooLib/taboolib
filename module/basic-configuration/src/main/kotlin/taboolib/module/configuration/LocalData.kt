 ...
 */
fun createTempData(path: String, saveTime: Long = 1200, type: Type? = null): Configuration {
    if (fileMap.containsKey(path)) {
        return fileMap[path]!!
    }
    if (init) {
        init = false
        submitAsync(period = saveTime) { LocalData.saveAll() }
    }
    return fileMap.computeIfAbsent(path) { Configuration.loadFromFile(newFile(getDataFolder(), path, create = true), type) }
}

@Deprecated("Use createTempData instead.", ReplaceWith("createTempData(path, saveTime, type)"))
fun createLocal(path: String, saveTime: Long = 1200, type: Type? = null): Configuration {
    return createTempData(path, saveTime, type)
}
