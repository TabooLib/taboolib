...
/**
 * TabooLib
 * starslib.platform.type.NukkitPlayer
 *
 * @author CziSKY
...
class NukkitPlayer(val player: Player) : ProxyPlayer {
    ...
    override var compassTarget: Location
        get() = player.spawn.location.toCommonLocation()
        set(_) {
            error("unsupported")
        }
    ...
    override var gameMode: ProxyGameMode
        get() = ProxyGameMode.fromString(player.gamemode.toString())
        set(value) {
            player.gamemode = value.ordinal
        }
    ...
    override fun playSound(location: Location, sound: String, volume: Float, pitch: Float) {
        player.level.addChunkPacket(player.chunk.x, player.chunk.z, PlaySoundPacket().also { packet ->
            ...
            packet.pitch = pitch
            packet.name = sound
            packet.x = location.x.toInt()
            packet.y = location.y.toInt()
            packet.z = location.z.toInt()
        })
    ...
    override fun sendParticle(particle: ProxyParticle, location: Location, offset: taboolib.common.util.Vector, count: Int, speed: Double, data: ProxyParticle.Data?) {
    ...
    override fun teleport(loc: Location) {
        val level = NukkitPlugin.getInstance().server.getLevelByName(loc.world) ?: player.level
        val location = cn.nukkit.level.Location.fromObject(Vector3(location.x, location.y, location.z), level, location.yaw.toDouble(), location.pitch.toDouble())
