...
open class Query(val table: Table, var dataSource: DataSource? = null) {
    ...
    open fun select(func: ActionSelect.() -> Unit) {
        ...
        task = executeQuery(action.query)
    }
    ...
    open fun executeQuery(query: String, hook: QueryHook? = null): QueryTask {
        ...
        return QueryTask(object : Future<ResultSet> {
            ...
            override fun <C> call(func: ResultSet.() -> C): C {
                return dataSource!!.connection.use {
                    ...
                    hook?.finishFunc?.invoke(prepareStatement, this@use)
    ...
    open fun executeUpdate(query: String, hook: QueryHook? = null): QueryTask {
        dataSource ?: error("no datasource")
        return RunTask(object : Future<ResultSet> {
            ...
            override fun <C> call(func: ResultSet.() -> C): C {
                return dataSource!!.connection.use {
                    val prepareStatement = prepareStatement(query, autoGeneratedKeys)
                    hook?.finishFunc?.invoke(prepareStatement, this@use)
                    prepareStatement.executeUpdate() as C
                }
            }
        })
    }
