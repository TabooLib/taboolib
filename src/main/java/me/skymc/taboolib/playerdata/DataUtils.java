...
public class DataUtils implements Listener {
	...
	public static void saveAllCahces(Plugin plugin) {
	...
	public static FileConfiguration getPluginData(String name, Plugin plugin) {
		return !CACHE_DATA_PLUGIN.containsKey(getDataSaveKey(plugin)) ? null : CACHE_DATA_PLUGIN.get(getDataSaveKey(plugin)).get(getFixedFileName(name));
	}
