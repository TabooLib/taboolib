package io.izzel.taboolib.metrics;
...
public class CStats {
    ...
    static {
        ...
        if (System.getProperty("cStats.relocatecheck") == null || !System.getProperty("cStats.relocatecheck").equals("false")) {
            ...
            if (CStats.class.getPackage().getName().equals(defaultPackage) || CStats.class.getPackage().getName().equals(examplePackage)) {
                throw new IllegalStateException("cStats cStats class has not been relocated correctly!");
            }
        }
    }
    ...
    public CStats(Plugin plugin) {
        ...
        if (enabled) {
            boolean found = false;
            // Search for all other cStats cStats classes to see if we are the first one
    ...
    private void submitData() {
        ...
        // Search for all other cStats cStats classes to get their plugin data
        for (Class<?> service : Bukkit.getServicesManager().getKnownServices()) {
            try {
                service.getField("C_STATS_VERSION"); // Our identifier :)
